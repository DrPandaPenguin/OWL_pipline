...
_EDGE_REFINE_SYSTEM = _load_prompt("edge_refine_system", (
    ...
    "You review an already-extracted set of edges and improve graph quality "
    "by removing wrong/redundant edges and adding important missing ones. "
    "Return strict JSON only."
))

_EDGE_REFINE_USER_TEMPLATE = _load_prompt("edge_refine_user", """\
Review the knowledge graph below and suggest improvements.
...
TRANSCRIPT (for context and evidence):
---
{transcript}
---
...
PART 1 — REMOVE: List edge_ids that should be removed because they are:
  - Factually wrong (the relationship does not hold in the lecture)
  - Hallucinated (no evidence in the transcript)
  - Self-loop (from == to)
  - Exact duplicate of another edge

PART 2 — ADD: List edges that are clearly missing. For each:
  - Must connect two existing node IDs (from/to)
  - Must have a justification_span traceable to the transcript
  - Must have confidence_score 0.0–1.0
  - Must use one of the allowed edge types

Return strict JSON:
{{
  "remove_edge_ids": ["edge_003", "edge_007"],
  "add_edges": [
    {{
      "from": "node_002",
      "to": "node_005",
      "edge_type": "requires",
      "justification_span": "Short quote or paraphrase from transcript",
...
Rules:
- Only reference node IDs from the NODES list above.
- If nothing to remove, set remove_edge_ids to [].
- If nothing to add, set add_edges to [].
- Do NOT re-add edges that are already in CURRENT EDGES (even if unlisted in remove).
- Be conservative: only remove when clearly wrong; only add when clearly supported.\
""")
...
def refine_edges(transcript, nodes, edges, config=None):
    ...
    for e in edges:
        ...
        just = e.get("justification_sentence") or e.get("justification_span", "")
        conf = e.get("confidence_score")
        conf_str = f" [conf={conf:.2f}]" if conf is not None else ""
        edge_lines.append(f"  [{eid}] {fr} --{etype}--> {to}{conf_str}  | {just[:80]}")
    edges_text = "\n".join(edge_lines)

    prompt = _EDGE_REFINE_USER_TEMPLATE.format(
    ...
    try:
        response = client.chat.completions.create(
            model=cfg.get("soft_model", "gpt-5.2"),
            messages=[
                {"role": "system", "content": _EDGE_REFINE_SYSTEM},
                {"role": "user", "content": prompt},
            ],
            response_format={"type": "json_object"},
            temperature=cfg.get("refine_temperature", 0.1),
        )
        result = json.loads(response.choices[0].message.content)
    except Exception as e:
        print(f"refine_edges error: {e}", file=sys.stderr)
        return edges

    remove_ids = set(result.get("remove_edge_ids", []))
    add_raw = result.get("add_edges", [])

    node_ids = {_node_id(n) for n in nodes if _node_id(n)}
    valid_types = set(edge_types)
    kept = [e for e in edges if e.get("edge_id") not in remove_ids]
    existing_keys = {
        (e.get("from") or e.get("source_node"),
         e.get("to") or e.get("target_node"),
         e.get("edge_type"))
    ...
    for raw in add_raw:
        ...
        if key in existing_keys:
            continue
        try:
            conf = max(0.0, min(1.0, float(e.get("confidence_score", 0.7))))
        except (TypeError, ValueError):
            conf = 0.7
        e["confidence_score"] = conf
        e.setdefault("justification_span", e.get("justification_sentence", ""))
        kept.append(e)
...
def validate_edge(edge, nodes, config=None):
    cfg = config or {}
    node_ids = {_node_id(n) for n in nodes if _node_id(n)}
    fr = edge.get("from") or edge.get("source_node")
    to = edge.get("to") or edge.get("target_node")
    ...
    if edge.get("edge_type") not in set(cfg.get("edge_types", _DEFAULT_EDGE_TYPES)):
        return False
    if "justification_sentence" in edge:
        return bool(edge.get("justification_sentence", "").strip())
    if "justification_span" in edge:
        return bool(edge.get("justification_span", "").strip()) and (
            edge.get("confidence_score") is not None or edge.get("confidence") == "soft"
        )
    return False
